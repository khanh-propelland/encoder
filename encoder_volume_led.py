...
class Encoder:
    ...
    def mapToVolume(self):
        ...
        if rawpos == 0:
            volume = 0
            VOLUME = 0
        elif rawpos in range (1, 96):
            VOLUME = math.ceil(rawpos * (100/96))
            volume = math.ceil (a * math.log((b *rawpos), 2))
        elif rawpos in range (96,112): # sets the beginning range before stop to zero
            volume = 100
            VOLUME = 100
        elif rawpos in range (112, 128):
            volume = 0
            VOLUME = 0
##            
##        if rawpos == 112:
##            volume = 0
##        elif rawpos in range (0, 80):
##            x = rawpos + 16
##            #volume = math.ceil (a * math.exp (b *x))
##            volume = math.ceil (a * math.log((b *x), 2))
##            #volume =  math.ceil( ( (rawpos + 16) ) * (volSpan / posSpan) )
##        elif rawpos in range (80, 96): # sets the end range after stop to 100
##            volume = 100
##        elif rawpos in range (96,112): # sets the beginning range before stop to zero
##            volume = 0
##        elif rawpos in range (113, 128):
##            x = rawpos - 112
##            #volume = math.ceil(a * math.exp (b *x) )
##            volume = math.ceil (a * math.log((b *x), 2))
##            #volume =  math.ceil(  ((rawpos - 112) ) *  (volSpan / posSpan))
...
if __name__ == "__main__":
    ...
    starting_volume = encoder.mapToVolume()
    v = Volume(volume=starting_volume)
